// Границы карты (ограничим область — Канада, США, Мексика)
const worldBounds = L.latLngBounds(
  L.latLng(18, -161),   // юг — немного южнее Гонолулу
  L.latLng(51, -30)     // север и восток — охватывает США, но немного Канады и минимум Мексики
);

// Создаем карту с ограничениями
const map = L.map('map', {
  maxBounds: worldBounds,
  maxBoundsViscosity: 1,      // сильнее прилипание к границе
  minZoom: 4,
  maxZoom: 18,
  worldCopyJump: false,
  zoomControl: false,
...
}).setView([38, -97], 5); // Центр США, масштаб 4
...
// Иконка для маркеров столиц (с звездой)
...
function createPopupContent(feature) {
  ...
  // Добавляем остальные свойства, исключая служебные и capital
  const propsToShow = Object.keys(feature.properties)
    .filter(k => !['stroke', 'stroke-width', 'stroke-opacity', 'fill', 'fill-opacity', 'flag', 'z-index', 'capital'].includes(k));
...
fetch('map.geojson')
  ...
  .then(data => {
    ...
    const colorDataMap = {};
    const ignoredKeys = ['stroke', 'stroke-width', 'stroke-opacity', 'fill', 'fill-opacity', 'flag', 'z-index', 'capital'];
    ...
    L.geoJSON({ type: "FeatureCollection", features: processed }, {
      pointToLayer: (feature, latlng) => {
        // Проверяем, является ли маркер столицей
        const isCapital = feature.properties && 
